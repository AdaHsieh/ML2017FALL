...
def valid1(X_valid, Y_valid, mean1,mean2,var,number1,number2):
    x = X_valid.T
    w = np.dot((mean1-mean2),np.linalg.inv(var))
    b = (-0.5)*np.dot(mean1,np.dot(np.linalg.inv(var),mean1))+(0.5)*np.dot(mean2,np.dot((np.linalg.inv(var)),mean2))+ math.log(float(number1/number2))
    a = np.dot(w, x) + b
    y = sigmoid(a)
    y_ = np.around(y)
    result = (np.squeeze(Y_valid) == y_)
...
def infer(X_test, save_dir, output_dir):
    ...
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    output_path = os.path.join(output_dir, 'generative_prediction.csv')
...
def main(*args):
    ...
    #output_dir = "/Users/adahsieh/Desktop/"
    output_dir = args[0][6]
